...
import (
	...
	"github.com/Overlrd/snippetbox/internal/assert"
)

func TestCommomHeaders(t *testing.T) {
	...
	// Pass the mock HTTP handler to the commonHeaders middleware.
	commonHeaders(next).ServeHTTP(rr, r)

	//
	rs := rr.Result()

	// Check if the middleware has correctly set the Content-Security-Policy
	...
	// Check that the middleware has correctly set the server
	// header on the response
	expectedValue = "Go"
